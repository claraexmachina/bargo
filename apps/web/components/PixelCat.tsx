...
type CatVariant = 'seller' | 'buyer';

interface PixelCatProps extends Omit<SVGProps<SVGSVGElement>, 'role'> {
  variant: CatVariant;
}
 ...
 */
export function PixelCat({ variant, className, ...props }: PixelCatProps) {
  const body = variant === 'seller' ? '#FFD700' : '#FFC3C3';
  const shade = variant === 'seller' ? '#CCAC00' : '#E89292';
  const ink = '#353B51';
  ...
  return (
    <svg
      ...
      className={className}
      style={{ transform: variant === 'buyer' ? 'scaleX(-1)' : undefined }}
      aria-hidden="true"
      {...props}
    >
      {pixels.map(([x, y, c]) => (
